            ...
            user.save((err,user)=>{
                if(err)
                 {throw err;}
                    ...
                    Smile.find({receiver:req.user._id,receiverReceived:false})
                    .then((newSmile)=>{
                       Chat.find({$or:[
...
app.get('/userProfile/:id',requireLogin,(req,res)=>{
    ...
    .then((user)=>{
       Smile.find({receiver:req.params.id})
       .then((smile)=>{
...
app.get('/startChat/:id',requireLogin,(req,res)=>{
    Chat.find({sender: req.params.id, receiver: req.user._id})
    .then((chat)=>{
        if(chat) {
            ...
            Chat.find({sender: req.user._id,receiver: req.params.id})
            .then((chat)=>{
            if(chat) {
...
app.post('/chat/:id',requireLogin,walletChecker,(req,res)=>{
    Chat.find({_id:req.params.id, sender:req.user._id})
    .populate('sender')
    .populate('receiver')
    .populate('chats.senderName')
    .populate('chats.receiverName')
    .then((chat)=>{
        ...
        // receiver message
            ...
            Chat.find({_id: req.params.id,receiver: req.user._id})
            .populate('sender')
            .populate('receiver')
            .populate('chats.senderName')
            .populate('chats.receiverName')
